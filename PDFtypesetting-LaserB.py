...
class mainwindow(QMainWindow):
    ...
    def add_File(self):
        ...
        for md in range(tableView1_model.rowCount()):
            check_width_height.add(
                self.ui.open_tableView1.model().index(md, 2).data() + '*' + self.ui.open_tableView1.model().index(md,
                                                                                                                  3).data())
            print("test:", self.ui.open_tableView1.model().index(md, 2).data())
    ...
    def getPDF_page_count(self,filepath):
        ...
        re2 = re.compile('.*MAIN.*_job(\d{0,5})\.PDF', re.I)  # _job PDF檔
    ...
    def open_folder(self):
        ...
        print("folder:",folder)
        re1 = re.compile('(.*? Blank Printed sheet.*\.pdf)', re.I)  # Blank Printed sheet PDF檔
        re2 = re.compile('(_job\d{1,5}\.pdf)', re.I)  # _job PDF檔
        re3 = re.compile('(\d{7}-\d{7}\.pdf)', re.I)  # 0000001-0004000.pdf檔
...
def combine(list1):
    ...
    def doutimes(doc_process):
        ...
        if bleeding_type[0] == "1" or bleeding_type[0] == "3":
            ...
            Length = width
            rotate = 180
        if  bleeding_type[0] == "5" or bleeding_type[0] == "7":
            layout = width_layout
            bleeding = bleeding_w
            Length = width
            rotate = 0
    ...
    for l1 in range(1,pdf_file_page_qty+1):     #标准PDF档数量
        if addart != 'no_art':  #如果有稿件.
            ...
            if background.page_count > 1 and check_addblank2 =='False':  # 稿件有兩頁并且不是單面加稿件底頁
                ...
                # doc[l1-1].show_pdf_page(r1, background, 0, keep_proportion=1)
                doc[l1 - 1].show_pdf_page(r1, background, 1, keep_proportion=1)     #打印是在第二頁的.
        else:
            doc.new_page(-1, width=width_total, height=height_total)
        ...
        for l2 in range(1,(cnt_page*std_pdf_file_count2)+1):
            ...
            if addart != 'no_art':  # 如果有稿件.
                ...
                if background.page_count > 1 and check_addblank2 =='False':  # 稿件有兩頁并且不是單面加稿件底頁
                    ...
                    # doc2[l2 - 1].show_pdf_page(r1, background, 0, keep_proportion=1)
                    doc2[l2 - 1].show_pdf_page(r1, background, 1, keep_proportion=1)    #打印在第二頁的
            else:
                page2=doc2.new_page(-1, width=width_total, height=height_total)
        ...
        for l3 in range(1,cnt_page_end+1):
            ...
            if addart != 'no_art':  # 如果有稿件.
                ...
                if background.page_count > 1 and check_addblank2 =='False':  # 稿件有兩頁并且不是單面加稿件底頁
                    ...
                    # doc3[l3 - 1].show_pdf_page(r1, background, 0, keep_proportion=1)
                    doc3[l3 - 1].show_pdf_page(r1, background, 1, keep_proportion=1)    #打印在第二頁的
            else:
                page3 = doc3.new_page(-1, width=width_total, height=height_total)
            ...
            if rec2 % pdf_file_qty == 0:    #標準箱整個檔案處理
                ...
                if check_addblank2 =='True':    #單面加空白底頁
                    ...
                    for i in range(int(pagecount)):
                        ...
                        page1.show_pdf_page(r1, background, 0, keep_proportion=1)   #不打印的在第一頁
